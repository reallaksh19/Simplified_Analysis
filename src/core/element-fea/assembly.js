...
export function assembleContinuumSystem(model, loadCase) {
  ...
  const operators = model.elements.map((element) => {
    ...
    return { element, operator, indices };
  });
  const loadEvidence = assembleLoads(model, loadCase, lookup, nodeMap);
  const assembledSystemHash = semanticHash({ dofMap, stiffness, appliedLoad: loadEvidence.appliedLoad });
...
export function partitionSystem(system, model) {
  ...
  const constrainedEquations = constraints.map((row) => row.equation);
  const constrainedSet = new Set(constrainedEquations);
  const freeEquations = system.dofMap.map((row) => row.equation).filter((equation) => !constrainedSet.has(equation));
  const prescribedValues = constraints.map((row) => row.value);
  const Kff = submatrix(system.stiffness, freeEquations, freeEquations);
  const Kfc = submatrix(system.stiffness, freeEquations, constrainedEquations);
  const freeAppliedLoad = freeEquations.map((index) => system.appliedLoad[index]);
  const imposedDisplacementLoad = multiplyMatrixVector(Kfc, prescribedValues);
  const effectiveFreeLoad = subtractVectors(freeAppliedLoad, imposedDisplacementLoad);
  return deepFreeze({
    constraints,
    freeEquations,
...
export function reconstructDisplacement(size, partition, freeValues) {
  ...
  partition.freeEquations.forEach((equation, index) => { displacement[equation] = freeValues[index]; });
  partition.constrainedEquations.forEach((equation, index) => { displacement[equation] = partition.prescribedValues[index]; });
  return displacement;
}
...
function assembleLoads(model, loadCase, lookup, nodeMap) {
  ...
  const directNodalLoadEvidence = loadCase.nodalForces.map((load) => {
    const uxEquation = lookup.get(`${load.nodeId}:UX`);
    const uyEquation = lookup.get(`${load.nodeId}:UY`);
    directNodalLoads[uxEquation] += load.fx;
    directNodalLoads[uyEquation] += load.fy;
    return { loadId: load.loadId, nodeId: load.nodeId, uxEquation, uyEquation, fx: load.fx, fy: load.fy };
  });
  const edgeLoadEvidence = loadCase.edgeLoads.map((load) => applyEdgeLoad(model, load, lookup, nodeMap, equivalentEdgeLoads));
...
function applyEdgeLoad(model, load, lookup, nodeMap, target) {
  ...
  return { loadId: load.loadId, elementId: element.elementId, type: load.type, ...evidence };
}
function constraintRows(model, lookup) {
  const rows = [
    ...model.restraints.map((row) => ({ ...row, constraintType: 'FIXED_ZERO', equation: lookup.get(`${row.nodeId}:${row.component}`) })),
    ...model.prescribedDisplacements.map((row) => ({ ...row, constraintType: 'PRESCRIBED', equation: lookup.get(`${row.nodeId}:${row.component}`) })),
  ];
