...
import { CONTINUUM_MODEL_SCHEMA, DOF_ORDER, EDGE_LOAD_TYPES, ELEMENT_TYPE, FORMULATIONS } from './constants.js';
...
export function createContinuumModel(input) {
  ...
  const loadCases = normalizeLoadCases(source.loadCases, sourceHash, nodes, elements, ownership);
  assertGlobalLoadIdentity(loadCases);
  const sourceReferences = normalizeSourceReferences(source.sourceReferences, sourceHash);
  ...
  const base = {
    ...
    solverProfileIdentity: profileIdentity,
  ...
  if (source.semanticHash !== undefined && source.semanticHash !== computedHash) throw new TypeError('Model semantic hash mismatch.');
  return deepFreeze({ ...base, semanticHash: computedHash });
}

function normalizeNodes(value, sourceHash) {
  const rows = array(value, 'nodes').map((value) => {
    const row = record(value, 'node');
    exactKeys(row, ['nodeId','x','y','sourceSemanticHash'], 'node');
    ancestry(row, sourceHash, 'node');
    return { nodeId: text(row.nodeId, 'nodeId'), x: finite(row.x, 'node.x'), y: finite(row.y, 'node.y'), sourceSemanticHash: sourceHash };
  });
...
function normalizeMaterials(value, sourceHash, formulation) {
  const rows = array(value, 'materials').map((value) => {
    const row = record(value, 'material');
    exactKeys(row, ['materialId','E','nu','sourceSemanticHash'], 'material');
    ancestry(row, sourceHash, 'material');
    const E = finite(row.E, 'material.E');
    const nu = finite(row.nu, 'material.nu');
    ...
    return { materialId: text(row.materialId, 'materialId'), E, nu, sourceSemanticHash: sourceHash };
  });
...
function elementRow(value, sourceHash, nodeMap, materialIds, profile) {
  const row = record(value, 'element');
  exactKeys(row, ['elementId','type','nodeIds','materialId','thickness','sourceSemanticHash','signedArea'], 'element');
  ancestry(row, sourceHash, 'element');
  if (row.type !== ELEMENT_TYPE) throw new TypeError(`Unsupported element type: ${row.type}.`);
  const nodeIds = array(row.nodeIds, 'element.nodeIds').map((id) => text(id, 'element.nodeId'));
  ...
  const thickness = profile.formulation === FORMULATIONS.PLANE_STRESS
    ? positive(row.thickness, 'element.thickness') : absent(row.thickness, 'Plane-strain element thickness');
  return { elementId: text(row.elementId, 'elementId'), type: ELEMENT_TYPE, nodeIds, materialId, thickness, signedArea: area, sourceSemanticHash: sourceHash };
}

function normalizeRestraints(value, sourceHash, nodes) {
  const nodeIds = new Set(nodes.map((row) => row.nodeId));
  const rows = array(value, 'restraints').map((value) => constraintRow(value, 'restraint', sourceHash, nodeIds, 0));
...
function normalizePrescribed(value, sourceHash, nodes, restraints) {
  const nodeIds = new Set(nodes.map((row) => row.nodeId));
  const rows = array(value, 'prescribedDisplacements').map((value) => {
    const source = record(value, 'prescribed displacement');
    return constraintRow(source, 'prescribed displacement', sourceHash, nodeIds, finite(source.value, 'prescribed.value'));
  });
...
function normalizeLoadCases(value, sourceHash, nodes, elements, ownership) {
  ...
  const rows = array(value, 'loadCases').map((value) => loadCaseRow(value, sourceHash, nodeIds, elementMap, ownership));
...
function loadCaseRow(value, sourceHash, nodeIds, elementMap, ownership) {
  const row = record(value, 'loadCase');
  exactKeys(row, ['loadCaseId','nodalForces','edgeLoads','sourceSemanticHash'], 'load case');
  ancestry(row, sourceHash, 'load case');
  const nodalForces = array(row.nodalForces, 'nodalForces').map((value) => nodalForce(value, sourceHash, nodeIds));
  const edgeLoads = array(row.edgeLoads, 'edgeLoads').map((value) => edgeLoad(value, sourceHash, elementMap, ownership));
  ...
  return {
    loadCaseId: text(row.loadCaseId, 'loadCaseId'),
    sourceSemanticHash: sourceHash,
    nodalForces: sortById(nodalForces, 'loadId'),
    edgeLoads: sortById(edgeLoads, 'loadId'),
  };
}

function nodalForce(value, sourceHash, nodeIds) {
  const row = record(value, 'nodalForce');
  exactKeys(row, ['loadId','nodeId','fx','fy','sourceSemanticHash'], 'nodal force');
  ancestry(row, sourceHash, 'nodal force');
  ...
  return { loadId: text(row.loadId, 'loadId'), nodeId, fx: finite(row.fx, 'nodalForce.fx'), fy: finite(row.fy, 'nodalForce.fy'), sourceSemanticHash: sourceHash };
}

function edgeLoad(value, sourceHash, elementMap, ownership) {
  const row = record(value, 'edgeLoad');
  const edgeAllowed = row.type === EDGE_LOAD_TYPES.TRACTION
    ? ['loadId','elementId','edgeNodeIds','type','tx','ty','sourceSemanticHash']
    : ['loadId','elementId','edgeNodeIds','type','pressure','sourceSemanticHash'];
  exactKeys(row, edgeAllowed, 'edge load');
  ancestry(row, sourceHash, 'edge load');
  ...
  if (edgeNodeIds.length !== 2 || new Set(edgeNodeIds).size !== 2 || edgeNodeIds.some((id) => !element.nodeIds.includes(id))) throw new TypeError('Edge load must reference two distinct nodes on a complete element edge.');
  ...
  if (!Object.values(EDGE_LOAD_TYPES).includes(row.type)) throw new TypeError('Unsupported edge load type.');
  const load = { loadId: text(row.loadId, 'loadId'), elementId, edgeNodeIds: [...edgeNodeIds].sort(compare), type: row.type, sourceSemanticHash: sourceHash };
  return row.type === EDGE_LOAD_TYPES.TRACTION
...
function normalizeSourceReferences(value, sourceHash) {
  const rows = array(value, 'sourceReferences').map((value) => {
    const row = record(value, 'source reference');
    exactKeys(row, ['sourceReferenceId','sourceType','sourceVersion','sourceSemanticHash'], 'source reference');
    ancestry(row, sourceHash, 'source reference');
    return {
  ...
  assertUnique(rows, 'sourceReferenceId', 'source reference');
  if (!rows.length) throw new TypeError('At least one source reference is required.');
  return sortById(rows, 'sourceReferenceId');
}
...
function constraintRow(value, label, sourceHash, nodeIds, valueNumber) {
  ...
  const allowed = label === 'restraint'
    ? ['constraintId','nodeId','component','sourceSemanticHash','value']
  ...
  return { constraintId: text(row.constraintId, `${label}.constraintId`), nodeId, component: row.component, value: valueNumber, sourceSemanticHash: sourceHash };
}
...
function adjacentEdge(nodes, edge) { return elementEdges(nodes).some((pair) => pair.includes(edge[0]) && pair.includes(edge[1])); }
