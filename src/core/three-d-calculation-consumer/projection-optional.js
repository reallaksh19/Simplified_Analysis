...
export function projectFlexuralProperties(projection) {
  return deepFreeze(projection.records.map((row) => deepFreeze({
    ...
    elasticModulusEvidence: deepFreeze({ valuePa: row.elasticModulusPa, sourceEvidence: evidenceByField(row.sourceEvidence, 'elastic') }),
    secondMomentEvidence: deepFreeze({ valueM4: row.secondMomentAreaM4, sourceEvidence: evidenceByField(row.sourceEvidence, 'moment') }),
    flexuralRigidityEvidence: deepFreeze({ valueNm2: row.flexuralRigidityNm2, sourceEvidence: row.sourceEvidence }),
    formulaTrace: row.formulaTrace || [],
    diagnostics: canonicalDiagnostics(row.diagnostics),
...
export function projectVerticalBeamCases(beamModel, solution, audit) {
  ...
  return deepFreeze(solution.pathCases.map((row) => {
...
function primitiveGeometry(row) {
  if ('startPoint' in row) return deepFreeze({ startPoint: row.startPoint, endPoint: row.endPoint, sourceLengthM: row.sourceLengthM });
  return deepFreeze({ applicationPoint: row.applicationPoint ?? null });
}
function primitiveMass(row) {
  if ('massPerLengthKgM' in row) return deepFreeze({ massPerLengthKgM: row.massPerLengthKgM, massSourceBreakdown: row.massSourceBreakdown || [] });
...
function primitiveMoment(row) { return 'momentMagnitudeNm' in row ? deepFreeze({ momentMagnitudeNm: row.momentMagnitudeNm, axisEvidence: row.axisEvidence }) : null; }
function nodeDisplacement(row) { return deepFreeze({ nodeId: row.nodeId, pathStationM: row.pathStationM, verticalDisplacementM: row.verticalDisplacementM, directionConvention: row.directionConvention }); }
function nodeRotation(row) { return deepFreeze({ nodeId: row.nodeId, pathStationM: row.pathStationM, rotationRad: row.rotationRad, directionConvention: row.directionConvention }); }
function caseKey(row) { return `${row.pathId}\0${row.loadCaseId}`; }
function evidenceByField(rows, token) { return (rows || []).filter((row) => JSON.stringify(row).toLowerCase().includes(token)); }
