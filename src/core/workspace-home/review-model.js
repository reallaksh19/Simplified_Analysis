...
const NOT_MIGRATED_IDS = Object.freeze(['PCF', 'SKETCHER', 'SETTINGS', 'QA', 'DEBUG']);
...
export function createWorkspaceHomeReviewModel(source) {
  ...
  const quickNavigation = deepFreeze(source.tabs
    .filter((row) => row.consumerId !== 'HOME' && row.available)
    .map((row) => deepFreeze({ consumerId: row.consumerId, label: row.label })));
  const pendingTabs = deepFreeze(source.tabs
    .filter((row) => NOT_MIGRATED_IDS.includes(row.consumerId))
  ...
  const base = {
    schema: WORKSPACE_HOME_REVIEW_MODEL_SCHEMA,
    sourceSemanticHash: source.semanticHash,
    applicationTitle: source.applicationTitle,
    dataset: deepFreeze({ status: source.datasetStatus, datasetId: source.datasetId }),
    counts,
    quickNavigation,
    pendingTabs,
    migrationStatement: 'PCF, Sketcher, Settings, QA, and Debug are visible in the recovered application structure but are not yet migrated to the current runtime.',
  };
