...
import { WORKSPACE_CONSUMER_EVENTS } from './workspace-consumer-controller.js';
...
export class ReportsConsumerController {
  ...
  init() {
    ...
    this.unsubscribeCallbacks = [
      this.eventBus.subscribe(WORKSPACE_CONSUMER_EVENTS.CHANGED, ({ context }) => this.handleContext(context)),
  ...
  handleExport(artifact) {
    this.status = { message: `Exported ${artifact.filename}` };
    this.render();
  }

  handleFailure(message) {
    this.status = { message: message || 'Report operation failed.' };
    this.render();
  }

  render() {
    if (!this.rootElement) return;
    const view = renderReportsConsumer(this.rootElement.ownerDocument, this.context, this.status);
  ...
  export(format) {
    this.eventBus.publish(MODEL_CALCULATION_EVENTS.EXPORT_REQUESTED, { format });
  }
...
export function renderReportsConsumer(documentRef, context, status = {}) {
  const section = documentRef.createElement('section');
  section.className = 'reports-consumer';
  section.dataset.role = 'reports-consumer';
  ...
  if (!report || !packageValue || !ledger) {
    section.innerHTML = unavailableMarkup();
    return section;
  }
  section.innerHTML = availableMarkup(report, packageValue, ledger, status);
  return section;
}

function availableMarkup(report, packageValue, ledger, status) {
  const sections = report.sections;
  return `
    <header class="reports-consumer__header">
      <div><span class="panel-eyebrow">Archived model calculation evidence</span><h1>Reports</h1></div>
      <p class="reports-consumer__disclaimer">${escapeHtml(report.statement)}</p>
    </header>
    ...
    ${methodSection(sections.methods)}
    ${listSection('Assumptions', sections.assumptions)}
    ${listSection('Limitations', sections.limitations)}
    ${qualificationSection(sections.qualification)}
    ${screeningSection(sections.screeningSupportForces)}
    ${beamSection(sections.verticalBeamSupportForces)}
    ${residualSection(sections.residualEvidence)}
    ${listSection('Blockers', sections.blockers)}
    ${diagnosticSection(sections.diagnostics)}
    <footer class="reports-consumer__footer">${escapeHtml(report.statement)}</footer>`;
}

function identitySection(identity) {
  const rows = [
    ['Package ID', identity.packageId], ['Package mode', identity.packageMode],
    ['Dataset ID', identity.datasetId], ['Ledger entry', identity.entryId],
    ['Archive key', identity.archiveKey], ['Sequence', identity.sequence],
  ];
  return `<section class="reports-card" data-role="reports-package-identity"><h2>Package identity</h2>${definitionList(rows)}</section>`;
}

function methodSection(methods) {
  const rows = methods.map((row) => `<tr><td>${escapeHtml(row.methodId)}</td><td>${escapeHtml(row.methodVersion)}</td><td>${escapeHtml(row.engineeringLevel)}</td><td>${escapeHtml(row.profileId)}</td></tr>`).join('');
  return tableSection('Methods and engineering levels', 'reports-methods', ['Method ID', 'Version', 'Engineering level', 'Profile'], rows);
}
...
function listSection(title, rows) {
  const content = rows?.length ? `<ul>${rows.map((row) => `<li>${escapeHtml(textValue(row))}</li>`).join('')}</ul>` : '<p class="panel-empty">None recorded.</p>';
  return `<section class="reports-card"><h2>${escapeHtml(title)}</h2>${content}</section>`;
}

function tableSection(title, role, headings, rows) {
  const body = rows || `<tr><td colspan="${headings.length}">No evidence recorded.</td></tr>`;
  return `<section class="reports-card" data-role="${role}"><h2>${escapeHtml(title)}</h2><div class="reports-table-wrap"><table><thead><tr>${headings.map((item) => `<th>${escapeHtml(item)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div></section>`;
}

function definitionList(rows) {
  return `<dl>${rows.map(([key, value]) => `<div><dt>${escapeHtml(key)}</dt><dd>${escapeHtml(value ?? '—')}</dd></div>`).join('')}</dl>`;
}
function entryOptions(ledger) {
  return [...ledger.entries].reverse().map((row) => `<option value="${escapeHtml(row.entryId)}" ${row.entryId === ledger.activeEntryId ? 'selected' : ''}>#${row.sequence} · ${escapeHtml(row.packageId)}</option>`).join('');
}
function unavailableMarkup() {
  return '<header class="reports-consumer__header"><div><span class="panel-eyebrow">Archived model calculation evidence</span><h1>Reports</h1></div></header><section class="reports-card" data-role="reports-unavailable"><h2>Report unavailable</h2><p class="panel-empty">Create and select a validated model calculation package in Workspace before opening Reports.</p></section>';
}
function textValue(value) {
  if (typeof value === 'string') return value;
  if (value && typeof value === 'object') return value.message || value.code || JSON.stringify(value);
  return String(value ?? '');
}
function number(value) { return Number.isFinite(value) ? String(value) : '—'; }
function escapeHtml(value) { return String(value ?? '').replace(/[&<>'"]/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[char])); }
function bind(view, action, callback) {
  view.querySelector(`[data-reports-action="${action}"]`)?.addEventListener('click', callback);
}
