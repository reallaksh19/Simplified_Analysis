...
export class ApplicationShellController {
  constructor(rootElement, consumerController, eventBus = EventBus, pipeSolverAdapter = null) {
    ...
    this.elementFeaController = new ElementFeaConsumerController(
      rootElement?.querySelector('[data-role="element-fea-consumer-root"]'),
    );
  ...
  handleDatasetReplacement() {
    if (this.state.activeViewId === 'WORKSPACE' || this.state.activeViewId === 'ELEMENT_FEA') return;
    const previous = this.state.activeViewId;
    this.state = createApplicationViewStateV5(this.readiness, {
      activeViewId: 'WORKSPACE', version: this.state.version + 1,
    });
...
export class ApplicationShellView {
  constructor(rootElement, eventBus) {
    ...
    this.views = new Map(['WORKSPACE','REPORTS','LOAD_CALC','THREE_D_CALC','PIPE_SOLVER','ELEMENT_FEA'].map((id) => [
