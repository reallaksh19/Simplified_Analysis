export function renderLoadCalcConsumer(documentRef, model, status = {}) {
  const section = documentRef.createElement('section');
  section.className = 'load-calc-consumer';
  section.dataset.role = 'load-calc-consumer';
  section.innerHTML = model ? availableMarkup(model, status) : unavailableMarkup(status);
  return section;
}

function availableMarkup(model, status) {
  return `
    ...
    <section class="load-calc-consumer__controls" aria-label="Load Calc actions">
      ${action('rebuild-model-loads','Rebuild Model Loads')}
      ${action('export-model-loads','Export Model Loads')}
      ${action('rebuild-paths','Rebuild Vertical Load Paths')}
      ${action('run-screening','Run Tributary Screening')}
      ${action('export-screening','Export Tributary Screening',!model.summary.screeningIncluded)}
...
function primitiveSection(rows) {
  const body = rows.map((row)=>`<tr>
    ...
    <td>${escapeHtml(geometry(row))}</td><td>${escapeHtml(mass(row))}</td><td>${escapeHtml(force(row))}</td><td>${escapeHtml(moment(row))}</td><td>${escapeHtml(vector(row.globalVector))}</td><td>${trace(row.formulaTrace)}</td><td>${diagnostics(row.diagnostics)}</td>
  </tr>`).join('');
  return tableCard('Primitive evidence','load-calc-primitives',['Primitive','Case','Component','Type','Geometry/application','Mass evidence','Force evidence','Moment evidence','globalVector','Formula trace','Diagnostics'],body);
}
...
function action(name, label, disabled=false) { return `<button type="button" data-load-calc-action="${name}" ${disabled?'aria-disabled="true"':''}>${escapeHtml(label)}</button>`; }
