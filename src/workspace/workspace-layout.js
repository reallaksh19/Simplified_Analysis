...
function workspaceMarkup() {
  return `<div class="application-shell" data-role="application-shell">
    ...
    <main class="workspace-shell" aria-label="Analysis Workspace">
      ...
      <div class="application-view application-view--element-fea" data-application-view="ELEMENT_FEA" hidden aria-hidden="true"><div data-role="element-fea-consumer-root"></div></div>
    </div>`;
}
function shellStyles() {
  return `
    ...
    .application-view--reports,.application-view--load-calc,.application-view--three-d-calc,.application-view--pipe-solver,.application-view--element-fea{overflow:auto;background:var(--workspace-canvas)}
    .reports-consumer,.load-calc-consumer,.three-d-calc-consumer,.pipe-solver-consumer,.element-fea-consumer{display:grid;gap:14px;max-width:1500px;margin:0 auto;padding:18px;min-width:0}
    .reports-consumer__header,.load-calc-consumer__header,.three-d-calc-consumer__header,.pipe-solver-consumer__header,.element-fea-consumer__header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.reports-consumer__header h1,.load-calc-consumer__header h1,.three-d-calc-consumer__header h1,.pipe-solver-consumer__header h1,.element-fea-consumer__header h1{margin:4px 0 0}
    .reports-consumer__disclaimer,.reports-consumer__footer,.load-calc-consumer__claim,.three-d-calc-consumer__claim,.pipe-solver-consumer__claim,.element-fea-consumer__claim{max-width:620px;color:#fbbf24;font-weight:700}
    .reports-consumer__controls,.load-calc-consumer__controls,.three-d-calc-consumer__controls,.pipe-solver-consumer__controls,.element-fea-consumer__controls{display:flex;flex-wrap:wrap;align-items:end;gap:8px;padding:12px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}
    ...
    .reports-consumer__controls select,.reports-consumer__controls button,.load-calc-consumer__controls button,.three-d-calc-consumer__controls button,.pipe-solver-consumer__controls button,.pipe-solver-consumer__controls select,.element-fea-consumer__controls button{border:1px solid #334155;border-radius:5px;padding:8px;background:#0b1628;color:var(--workspace-text)}
    .load-calc-consumer__controls button[aria-disabled="true"],.three-d-calc-consumer__controls button[aria-disabled="true"],.pipe-solver-consumer__controls button[aria-disabled="true"],.element-fea-consumer__controls button:disabled{color:#94a3b8;cursor:not-allowed}.reports-consumer__controls button:focus-visible,.reports-consumer__controls select:focus-visible,.load-calc-consumer__controls button:focus-visible,.three-d-calc-consumer__controls button:focus-visible,.pipe-solver-consumer__controls button:focus-visible,.pipe-solver-consumer__controls select:focus-visible,.element-fea-consumer__controls button:focus-visible{outline:2px solid rgba(96,165,250,.55);outline-offset:2px}
    .reports-consumer__controls output,.load-calc-consumer__controls output,.three-d-calc-consumer__controls output,.pipe-solver-consumer__controls output,.element-fea-consumer__controls output{flex-basis:100%;color:#bfdbfe}
    .reports-card,.load-calc-card,.three-d-calc-card,.pipe-solver-card,.element-fea-card{min-width:0;padding:14px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}.reports-card h2,.load-calc-card h2,.three-d-calc-card h2,.pipe-solver-card h2,.element-fea-card h2{margin:0 0 10px;font-size:14px}
    ...
    .element-fea-editor{display:grid;gap:6px}.element-fea-editor textarea{min-height:360px;resize:vertical;padding:12px;border:1px solid #334155;border-radius:5px;background:#060b14;color:var(--workspace-text);font-family:ui-monospace,monospace}.element-fea-card pre{max-height:520px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;background:#060b14;padding:12px}
