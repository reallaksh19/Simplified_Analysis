...
function shellStyles() {
  return `
    ...
    .application-view--reports,.application-view--load-calc{overflow:auto;background:var(--workspace-canvas)}
    .reports-consumer,.load-calc-consumer{display:grid;gap:14px;max-width:1500px;margin:0 auto;padding:18px;min-width:0}
    .reports-consumer__header,.load-calc-consumer__header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.reports-consumer__header h1,.load-calc-consumer__header h1{margin:4px 0 0}
    .reports-consumer__disclaimer,.reports-consumer__footer,.load-calc-consumer__claim{max-width:620px;color:#fbbf24;font-weight:700}
    .reports-consumer__controls,.load-calc-consumer__controls{display:flex;flex-wrap:wrap;align-items:end;gap:8px;padding:12px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}
    .reports-consumer__controls label{display:grid;gap:5px;min-width:280px;color:var(--workspace-muted);font-size:11px}
    .reports-consumer__controls select,.reports-consumer__controls button,.load-calc-consumer__controls button{border:1px solid #334155;border-radius:5px;padding:8px;background:#0b1628;color:var(--workspace-text)}
    .load-calc-consumer__controls button[aria-disabled="true"]{color:#94a3b8;cursor:help}.reports-consumer__controls button:focus-visible,.reports-consumer__controls select:focus-visible,.load-calc-consumer__controls button:focus-visible{outline:2px solid rgba(96,165,250,.55);outline-offset:2px}
    .reports-consumer__controls output,.load-calc-consumer__controls output{flex-basis:100%;color:#bfdbfe}
    .reports-card,.load-calc-card{min-width:0;padding:14px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}.reports-card h2,.load-calc-card h2{margin:0 0 10px;font-size:14px}
    .reports-card dl,.load-calc-card dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0}.reports-card dl div,.load-calc-card dl div{min-width:0;padding:8px;background:#060b14}.reports-card dt,.load-calc-card dt{color:var(--workspace-muted);font-size:11px}.reports-card dd,.load-calc-card dd{margin:4px 0 0;overflow-wrap:anywhere;font-weight:700}
    .reports-table-wrap,.load-calc-table-wrap{max-width:100%;overflow:auto}.reports-card table,.load-calc-card table{width:100%;border-collapse:collapse;font-size:12px}.reports-card th,.reports-card td,.load-calc-card th,.load-calc-card td{padding:8px;border-bottom:1px solid var(--workspace-border);text-align:left;vertical-align:top;overflow-wrap:anywhere}.reports-card th,.load-calc-card th{color:#bfdbfe;white-space:nowrap}.load-calc-card code{white-space:pre-wrap;overflow-wrap:anywhere}
    .visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
    @media(max-width:1100px){.application-navigation{flex-wrap:wrap}.reports-card dl,.load-calc-card dl{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:760px){.application-shell{height:auto;min-height:100vh;overflow:visible}.application-view{overflow:visible}.application-view--workspace .workspace-shell{height:auto}.reports-consumer__header,.load-calc-consumer__header{display:grid}.reports-card dl,.load-calc-card dl{grid-template-columns:minmax(0,1fr)}}`;
}
