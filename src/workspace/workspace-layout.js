...
function workspaceMarkup() {
  return `<div class="application-shell" data-role="application-shell">
      ...
      <div class="application-view application-view--home" data-application-view="HOME"><div data-role="home-consumer-root"></div></div>
      <div class="application-view application-view--workspace" data-application-view="WORKSPACE" hidden aria-hidden="true">
    <main class="workspace-shell" aria-label="Analysis Workspace">
      ...
      <div class="application-view application-view--pcf" data-application-view="PCF" hidden aria-hidden="true">${unavailableView('PCF','PCF intake and review is awaiting an authorized recovery increment.')}</div>
      <div class="application-view application-view--sketcher" data-application-view="SKETCHER" hidden aria-hidden="true">${unavailableView('Sketcher','Sketcher capability has not yet been migrated to the current runtime.')}</div>
      ...
      <div class="application-view application-view--settings" data-application-view="SETTINGS" hidden aria-hidden="true">${unavailableView('Settings','Settings authority has not yet been migrated to the current runtime.')}</div>
      <div class="application-view application-view--debug" data-application-view="DEBUG" hidden aria-hidden="true">${unavailableView('Debug','Debug is not implemented in the current runtime.')}</div>
    </div>`;
}
function unavailableView(title, message) { return `<section class="unavailable-view"><h1>${title}</h1><p>${message}</p></section>`; }
function shellStyles() {
  return `
    ...
    .application-navigation-shell{display:grid;border-bottom:1px solid var(--workspace-border);background:var(--workspace-panel-strong)}
    .application-navigation{display:flex;align-items:center;gap:6px;min-width:0;padding:8px 12px 4px;overflow-x:auto}
    .application-navigation__status{min-height:20px;padding:0 12px 6px;color:#fca5a5;font-size:12px}
    ...
    .application-view--home,.application-view--reports,.application-view--load-calc,.application-view--three-d-calc,.application-view--pipe-solver{overflow:auto;background:var(--workspace-canvas)}
    .home-consumer,.reports-consumer,.load-calc-consumer,.three-d-calc-consumer,.pipe-solver-consumer{display:grid;gap:14px;max-width:1500px;margin:0 auto;padding:18px;min-width:0}
    .home-consumer__header,.reports-consumer__header,.load-calc-consumer__header,.three-d-calc-consumer__header,.pipe-solver-consumer__header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.home-consumer__header h1,.reports-consumer__header h1,.load-calc-consumer__header h1,.three-d-calc-consumer__header h1,.pipe-solver-consumer__header h1{margin:4px 0 0}
    .home-consumer__header p,.reports-consumer__disclaimer,.reports-consumer__footer,.load-calc-consumer__claim,.three-d-calc-consumer__claim,.pipe-solver-consumer__claim{max-width:620px;color:#fbbf24;font-weight:700}
    .home-card,.reports-card,.load-calc-card,.three-d-calc-card,.pipe-solver-card{min-width:0;padding:14px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}.home-card h2,.reports-card h2,.load-calc-card h2,.three-d-calc-card h2,.pipe-solver-card h2{margin:0 0 10px;font-size:14px}
    .home-card dl,.reports-card dl,.load-calc-card dl,.three-d-calc-card dl,.pipe-solver-card dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0}.home-card dl div,.reports-card dl div,.load-calc-card dl div,.three-d-calc-card dl div,.pipe-solver-card dl div{min-width:0;padding:8px;background:#060b14}.home-card dt,.reports-card dt,.load-calc-card dt,.three-d-calc-card dt,.pipe-solver-card dt{color:var(--workspace-muted);font-size:11px}.home-card dd,.reports-card dd,.load-calc-card dd,.three-d-calc-card dd,.pipe-solver-card dd{margin:4px 0 0;overflow-wrap:anywhere;font-weight:700}
    .home-consumer__actions{display:flex;flex-wrap:wrap;gap:8px}.home-consumer__actions button{border:1px solid #334155;border-radius:5px;padding:8px 11px;background:#0b1628;color:var(--workspace-text);font-weight:700}.home-consumer__actions button:focus-visible{outline:2px solid rgba(96,165,250,.55);outline-offset:2px}
    .home-consumer__loading{padding:20px;color:var(--workspace-muted)}.unavailable-view{max-width:760px;margin:40px auto;padding:20px;border:1px solid var(--workspace-border);border-radius:7px;background:var(--workspace-panel)}
    ...
    .reports-consumer__controls label,.pipe-solver-consumer__controls label{display:grid;gap:5px;min-width:220px;color:var(--workspace-muted);font-size:11px}
    .reports-consumer__controls select,.reports-consumer__controls button,.load-calc-consumer__controls button,.three-d-calc-consumer__controls button,.pipe-solver-consumer__controls button,.pipe-solver-consumer__controls select{border:1px solid #334155;border-radius:5px;padding:8px;background:#0b1628;color:var(--workspace-text)}
    .load-calc-consumer__controls button[aria-disabled="true"],.three-d-calc-consumer__controls button[aria-disabled="true"],.pipe-solver-consumer__controls button[aria-disabled="true"]{color:#94a3b8;cursor:not-allowed}.reports-consumer__controls button:focus-visible,.reports-consumer__controls select:focus-visible,.load-calc-consumer__controls button:focus-visible,.three-d-calc-consumer__controls button:focus-visible,.pipe-solver-consumer__controls button:focus-visible,.pipe-solver-consumer__controls select:focus-visible{outline:2px solid rgba(96,165,250,.55);outline-offset:2px}
    ...
    .reports-table-wrap,.load-calc-table-wrap,.three-d-calc-table-wrap,.pipe-solver-table-wrap{max-width:100%;overflow:auto}.reports-card table,.load-calc-card table,.three-d-calc-card table,.pipe-solver-card table{width:100%;border-collapse:collapse;font-size:12px}.reports-card th,.reports-card td,.load-calc-card th,.load-calc-card td,.three-d-calc-card th,.three-d-calc-card td,.pipe-solver-card th,.pipe-solver-card td{padding:8px;border-bottom:1px solid var(--workspace-border);text-align:left;vertical-align:top;overflow-wrap:anywhere}.reports-card th,.load-calc-card th,.three-d-calc-card th,.pipe-solver-card th{color:#bfdbfe;white-space:nowrap}.load-calc-card code,.three-d-calc-card code,.pipe-solver-card code,.pipe-solver-card pre{white-space:pre-wrap;overflow-wrap:anywhere}.pipe-solver-card input{min-width:130px;border:1px solid #334155;border-radius:4px;padding:6px;background:#060b14;color:var(--workspace-text)}.pipe-solver-ledger-row--active{background:#1d2433}
    .visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
    @media(max-width:1100px){.application-navigation{flex-wrap:wrap;overflow-x:visible}.home-card dl,.reports-card dl,.load-calc-card dl,.three-d-calc-card dl,.pipe-solver-card dl{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:760px){.application-shell{height:auto;min-height:100vh;overflow:visible}.application-view{overflow:visible}.application-view--workspace .workspace-shell{height:auto}.home-consumer__header,.reports-consumer__header,.load-calc-consumer__header,.three-d-calc-consumer__header,.pipe-solver-consumer__header{display:grid}.home-card dl,.reports-card dl,.load-calc-card dl,.three-d-calc-card dl,.pipe-solver-card dl{grid-template-columns:minmax(0,1fr)}}`;
}
