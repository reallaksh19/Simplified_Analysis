...
export const clippingPlanes = [
    new THREE.Plane(new THREE.Vector3( 1,  0,  0), 10000), // MinX (normal points +X)
    new THREE.Plane(new THREE.Vector3(-1,  0,  0), 10000), // MaxX (normal points -X)
...
function createCylinder(startVec, endVec, radius, color) {
    ...
    const mat = new THREE.MeshStandardMaterial({ color, clippingPlanes, clipIntersection: false });
...
function createBox(pos, hw, color, wireframe = false) {
    ...
    const mat = wireframe
        ? new THREE.MeshBasicMaterial({ color, wireframe: true, clippingPlanes, clipIntersection: false })
        : new THREE.MeshStandardMaterial({ color, clippingPlanes, clipIntersection: false });
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.copy(pos);
    return mesh;
}
