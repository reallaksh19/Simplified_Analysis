...
function createCylinder(startVec, endVec, radius, color) {
    ...
    const mat = new THREE.MeshStandardMaterial({ color, clippingPlanes, side: THREE.DoubleSide });
...
function createBox(pos, hw, color, wireframe = false) {
    ...
    const mat = wireframe
        ? new THREE.MeshBasicMaterial({ color, wireframe: true, clippingPlanes, side: THREE.DoubleSide })
        : new THREE.MeshStandardMaterial({ color, clippingPlanes, side: THREE.DoubleSide });
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.copy(pos);
    return mesh;
}
