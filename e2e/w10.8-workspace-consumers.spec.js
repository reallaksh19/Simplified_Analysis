...
test('adopts archived W10.7 evidence with accessible deterministic navigation', async ({ page }) => {
  ...
  for(const label of NAVIGATION.slice(1)) {
    ...
    await expect(button).toBeEnabled(); await expect(button).toHaveAttribute('aria-disabled','true');
  ...
  await workspace.click(); await page.locator('[data-entity-id="PIPE-B"]').click();
  ...
  expect(await preservedState(page)).toEqual({...before,selectedEntityId:'PIPE-B',activeEntryId:beamEntry});
