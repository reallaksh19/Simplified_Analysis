...
for (const file of files) {
  ...
  try {
    if (parse) {
      ...
      skippedJsx.push(rel);
      if (strict) throw new Error('@babel/parser unavailable; strict JSX syntax check cannot run.');
    } else {
...
if (skippedJsx.length) {
  console.warn(`Syntax check fallback: @babel/parser unavailable, skipped JSX parse for ${skippedJsx.length} JSX file(s).`);
  console.warn('Run `npm install` then `npm run syntax:strict` or `npm run check:full` for final JSX/Babel QA.');
}
console.log(`Syntax check passed for ${checked - skippedJsx.length} parsed JS/MJS/CJS file(s). Total files scanned: ${checked}.`);
process.exit(0);
